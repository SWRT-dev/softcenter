...
# 			ln -sf /jffs/softcenter/scripts/frps_config.sh /jffs/softcenter/init.d/S99Frps.sh
		fi
	else
		if [ -L "/jffs/softcenter/init.d/N99Frps.sh" ];then
			echo_date "删除nat-start触发"
			rm -rf /jffs/softcenter/init.d/*99Frps.sh
		fi
	fi
}
del_crontab(){
...
fun_crontab(){
	if [ "${frps_enable}" != "1" ] || [ "${frps_cron_time}" == "0" ];then
		del_crontab
		return 1
	fi
	if [ "${frps_cron_hour_min}" == "min" ]; then
		if [ "${frps_cron_type}" == "watch" ]; then
			...
			cru a frps_monitor "*/"${frps_cron_time}" * * * * pidof frps >/dev/null || /jffs/softcenter/scripts/frps_config.sh watch"
		elif [ "${frps_cron_type}" == "start" ]; then
			echo_date "设置定时任务：每隔 ${frps_cron_time} 分钟 启动 一次frps服务"
			cru a frps_monitor "*/"${frps_cron_time}" * * * * /jffs/softcenter/scripts/frps_config.sh start"
		fi
	elif [ "${frps_cron_hour_min}" == "hour" ]; then
		if [ "${frps_cron_type}" == "watch" ]; then
			echo_date "设置定时任务：每隔 ${frps_cron_time} 小时 检查 一次frps服务"
			cru a frps_monitor "0 */"${frps_cron_time}" * * * pidof frps >/dev/null || /jffs/softcenter/scripts/frps_config.sh watch"
		elif [ "${frps_cron_type}" == "start" ]; then
			echo_date "设置定时任务：每隔 ${frps_cron_time} 小时 启动 一次frps服务"
			cru a frps_monitor "0 */"${frps_cron_time}" * * * /jffs/softcenter/scripts/frps_config.sh start"
		fi
	fi
}
# 生成frps运行日志文件链接和目录
logfile_prepare() {
	if [ "$frps_extra_config" != "1" ];then
		[ -n "${frps_common_log__to}" ] && RUN_logFile=${frps_common_log__to}
	else
	# 使用自定义参数时
		local a b c
		# For TOML
...
close_port(){
	...
	local tmp_file=/tmp/Clean_Frps_$$
...
# 开启进程
onRun(){
	echo_date "启动 frps 主程序..." | tee -a "${logFile_lnk}"
# 	export GOGC=40
...
onstart() {
	...
	if [ "$frps_extra_config" == "1" ];then
		...
		"log.to" "log.level" "log.maxDays..INT" "webServer.addr" "webServer.port..INT" "webServer.user" "webServer.password" "webServer.tls.certFile" "webServer.tls.keyFile"
	fi
