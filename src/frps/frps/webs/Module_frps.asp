...
function save() {
	...
	if (E("frps_extra_config").checked) {
		if (trim(E("frps_extra_options").value) == "") {
			alert("您已勾选“启用自定义”配置，表单不能为空!");
			return false;
		}
	}
	if( !trim(E("frps_common_auth__token").value) ){
		alert("令牌token 为“必填项”");
		return false;
	}
...
function hook_event(){
	...
	$("#frps_common_webServer__port").change(
		...
		if(E("frps_common_webServer__port").value == ""){
			...
			E("webServerCert_tr").style.display = "none";
			E("webServerKey_tr").style.display = "none";
		}else{
			E("webServerAddr_tr").style.display = "";
			E("webServerUser_tr").style.display = "";
			E("webServerPwd_tr").style.display = "";
			E("webServerCert_tr").style.display = "";
			E("webServerKey_tr").style.display = "";
		}
	});
}
...
function openssHint(itemNum){
	...
	if(itemNum == 0){
		...
		statusmenu ="是否开启日志保存。默认为空，即console<br/>①控制台（console），日志输出到标准输出。本插件会丢弃标准输出及错误信息，无法读取；<br/>②文件，日志保存到【/tmp/frps_run.log】。本插件会重定向标准输出及错误信息给它，均可读取；<br/>③黑洞（/dev/null），日志被丢弃。本插件仅重定向标准输出及错误信息至临时文件，可读取。";
		_caption = "log.to字段";
	}
	else if(itemNum == 7){
		...
		statusmenu ="启用后，输入<font color='#F46'>完整配置文件</font>，此时上表格参数将被忽略。视Frp版本而定，可能支持：INI/TOML/YAML/JSON 格式。INI/TOML/YAML 支持使用 # 注释符。具体参看frp官方文档。";
		_caption = "自定义配置";
	}
	else if(itemNum == 19){
		...
		statusmenu ="服务器管理面板TLS证书和密钥文件路径。可选项目，可尝试系统内置的 /etc/cert.pem 和 /etc/key.pem";
		_caption = "webServer.tls.certFile 和 webServer.tls.keyFile";
	}
					...
					<tr>
						<td align="left" valign="top">
							...
							<div id="frpslogfile"  class="contentM_qis" style="box-shadow: 3px 3px 10px #000;margin-top: 70px;">
								...
								<div style="margin-left:15px"><i>&nbsp;1、文本不会自动刷新，需配置日志输出且仅看当天。【某些系统下，日志显示时间比上海时间慢8小时】。</i></div>
								<div style="margin-left:15px"><i>&nbsp;2、若使用 “自定义配置” 方式，请注意：<br>&nbsp;&nbsp;&nbsp;①&nbsp;“键名” 关键词只出现一次，不要出现多次（无论是否注释）；<br>&nbsp;&nbsp;&nbsp;②&nbsp;“键值” 指定的文件路径在可写文件系统上。</i></div>
								<div id="loading_block_spilt" style="margin:10px 0 10px 5px;" class="loading_block_spilt"></div>
								<div style="margin-left:15px;margin-right:15px;margin-top:10px;overflow:hidden">
										...
										<div id="frps_main">
											<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable">
												...
												<tr>
													...
													<td>
														每
														<input type="text" oninput="this.value=this.value.replace(/[^\d]/g, '')" id="frps_cron_time" name="frps_cron_time" class="input_ss_table" style="width:30px" maxlength="2" value="0" placeholder="0" />
											...
											<table width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable" style="margin-top:8px;">
												...
												<tr>
													...
													<td>
														<input type="password" name="frps_common_auth__token" id="frps_common_auth__token" class="input_ss_table" autocomplete="new-password" autocorrect="off" autocapitalize="off" maxlength="75" value="" onBlur="switchType(this, false);" onFocus="switchType(this, true);" placeholder="必填项" />
													</td>
												</tr>
												<tr>
												...
												<tr>
													...
													<select id="frps_common_log__to" name="frps_common_log__to" style="width:165px;margin:0px 0px 0px 2px;" class="input_option" >
														<option value="">-空-</option>
														<option value="/tmp/frps_run.log">文件（/tmp/frps_run.log）</option>
												...
												<tr id="webServerCert_tr" style="display: none;">
													<th width="20%"><a class="hintstyle" href="javascript:void(0);" onclick="openssHint(24)">管理面板TLS证书</a></th>
													<td>
												<input type="text" class="input_ss_table" id="frps_common_webServer__tls__certFile" name="frps_common_webServer__tls__certFile" maxlength="99" value="" placeholder="可选" />
													</td>
												</tr>
												<tr id="webServerKey_tr" style="display: none;">
													<th width="20%"><a class="hintstyle" href="javascript:void(0);" onclick="openssHint(24)">管理面板TLS密钥</a></th>
													<td>
												<input type="text" class="input_ss_table" id="frps_common_webServer__tls__keyFile" name="frps_common_webServer__tls__keyFile" maxlength="99" value="" placeholder="可选" />
													</td>
												</tr>
												<tr>
												<th style="width:20%;"><a class="hintstyle" href="javascript:void(0);" onclick="openssHint(18)">自定义配置文件</a><br>
													<label><input type="checkbox" id="frps_extra_config" name="frps_extra_config"><i>启用自定义</i>
												</th>
												<td>
													<textarea cols="63" rows="10" wrap="off" id="frps_extra_options" name="frps_extra_options" autocomplete="off" autocorrect="off" autocapitalize="off" spellcheck="false" style="font-size:11px;background:#475A5F;color:#FFFFFF" placeholder="# 视Frp版本，可用格式（TOML/YAML/JSON/INI）&#13;"></textarea>
												</td>
											</tr>
