...
install_now(){
	# default value
	local TITLE="frps内网穿透"
	local DESCR="Frps路由器服务端，内网穿透利器。"
	local PLVER=$(cat ${DIR}/version)

	# stop before install
	if [ "$(dbus get frps_enable)" == "1" -a -f "/jffs/softcenter/scripts/frps_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/frps_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/bin/frps >/dev/null 2>&1
	rm -rf /jffs/softcenter/res/icon-frps.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/frps_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_frps.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_frps.asp >/dev/null 2>&1
	...
	[ ! -L "/jffs/softcenter/init.d/S99frps.sh" ] && ln -sf /jffs/softcenter/scripts/frps_config.sh /jffs/softcenter/init.d/S99frps.sh
	# Permissions
	...
	if [ "$(dbus get frps_enable)" == "1" -a -f "/jffs/softcenter/scripts/frps_config.sh" ];then
		echo_date "重新启动插件！"
		sh /jffs/softcenter/scripts/frps_config.sh start
	fi
