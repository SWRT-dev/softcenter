...
install_now(){
	# default value
	local TITLE="CPU频率设置"
	local DESCR="Intel CPU频率设置"
	local PLVER=$(cat ${DIR}/version)
	# stop before install
	if [ "$(dbus get cpufreq_enable)" == "1" -a -f "/jffs/softcenter/scripts/cpufreq_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/cpufreq_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/res/icon-cpufreq.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/cpufreq_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_cpufreq.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_cpufreq.asp >/dev/null 2>&1
	...
	[ ! -L "/jffs/softcenter/init.d/M40cpufreq.sh" ] && ln -sf /jffs/softcenter/scripts/cpufreq_config.sh /jffs/softcenter/init.d/M40cpufreq.sh
		# Permissions
	chmod +x /jffs/softcenter/bin/cpufreq*
	...
	if [ "$(dbus get cpufreq_enable)" == "1" -a -f "/jffs/softcenter/scripts/cpufreq_config.sh" ];then
		echo_date "重新开启插件..."
		/jffs/softcenter/scripts/cpufreq_config.sh start
	fi
