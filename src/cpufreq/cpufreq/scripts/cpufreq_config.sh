...
if [ -n "$cpufreq_set" ];then
	...
	[ "$cpufreq_set" -gt "$cpufreq_max" ] && echo_date "频率设置错误" >> /tmp/cpufreq.log && exit 1
	[ "$cpufreq_set" -lt 150 ] && echo_date "频率设置错误" >> /tmp/cpufreq.log && exit 1
	if [ "$cpufreq_set" -eq 667 ] ;then
		/jffs/softcenter/bin/cpufreq-set -f 666666
	else
		/jffs/softcenter/bin/cpufreq-set -f ${cpufreq_set}MHz
	fi
	echo_date "已设置频率：${cpufreq_set}MHz" >> /tmp/cpufreq.log
...
fun_restart() {
	if [ "$cpufreq_enable" == "1" ];then
		...
		echo_date "已开启自动频率设置" >> /tmp/cpufreq.log
		start
	else
		echo_date "插件未启用,已关闭自动频率设置" >> /tmp/cpufreq.log
		stop
	fi
}
