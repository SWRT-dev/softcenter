...
install_now(){
	# default value
	local TITLE="Speedtest"
	local DESCR="局域网网速测试工具"
	local PLVER=$(cat ${DIR}/version)

	# stop before install
	if [ "$(dbus get speedtest_enable)" == "1" -a -f "/jffs/softcenter/scripts/speedtest_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/speedtest_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/bin/speedtest >/dev/null 2>&1
	rm -rf /jffs/softcenter/res/icon-speedtest.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/speedtest_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_speedtest.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_speedtest.asp >/dev/null 2>&1
	find /jffs/softcenter/init.d -name "*speedtest*" | xargs rm -rf	
	...
	if [ "$(dbus get speedtest_enable)" == "1" -a -f "/jffs/softcenter/scripts/speedtest_config.sh" ];then
		echo_date "重新开启插件..."
		/jffs/softcenter/scripts/speedtest_config.sh start
	fi
