...
install_now(){
	# default value
	local TITLE="Socat端口转发"
	local DESCR="Socat 端口转发"
	local PLVER=$(cat ${DIR}/version)

	# stop before install
	if [ "$(dbus get socat_enable)" == "1" -a -f "/jffs/softcenter/scripts/socat_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/socat_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/res/icon-socat.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/socat_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_socat.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_socat.asp >/dev/null 2>&1
	...
	[ ! -L "/jffs/softcenter/init.d/N99socat.sh" ] && ln -sf /jffs/softcenter/scripts/socat_config.sh /jffs/softcenter/init.d/N99socat.sh
	# Permissions
	...
	if [ "$(dbus get socat_enable)" == "1" -a -f "/jffs/softcenter/scripts/socat_config.sh" ];then
		echo_date "重新开启插件..."
		/jffs/softcenter/scripts/socat_config.sh start
	fi
