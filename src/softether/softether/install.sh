...
install_now(){
	# default value
	local TITLE="softether"
	local DESCR="VPN全家桶"
	local PLVER=$(cat ${DIR}/version)

	# stop before install
	if [ "$(dbus get softether_enable)" == "1" -a -f "/jffs/softcenter/scripts/softether_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/softether_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/bin/vpnserver >/dev/null 2>&1
	rm -rf /jffs/softcenter/bin/hamcore.se2 >/dev/null 2>&1
	rm -rf /jffs/softcenter/res/icon-softether.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/softether_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_softether.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_softether.asp >/dev/null 2>&1
...
#	[ ! -L "/jffs/softcenter/init.d/S98softether.sh" ] && ln -sf /jffs/softcenter/scripts/softether_config.sh /jffs/softcenter/init.d/S98softether.sh
	[ ! -L "/jffs/softcenter/init.d/N98softether.sh" ] && ln -sf /jffs/softcenter/scripts/softether_config.sh /jffs/softcenter/init.d/N98softether.sh
	# Permissions
