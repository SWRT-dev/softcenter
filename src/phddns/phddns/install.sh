...
install_now(){
	...
	# stop before install
	if [ "$(dbus get phddns_enable)" == "1" -a -f "/jffs/softcenter/scripts/phddns_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/phddns_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/bin/phtunnel >/dev/null 2>&1
	rm -rf /jffs/softcenter/res/icon-phddns.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/phddns_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_phddns.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_phddns.asp >/dev/null 2>&1
	find /jffs/softcenter/init.d -name "*phddns*" | xargs rm -rf
	...
	[ ! -L "/jffs/softcenter/init.d/S99phddns.sh" ] && ln -sf /jffs/softcenter/scripts/phddns_config.sh /jffs/softcenter/init.d/S99phddns.sh
	# Permissions
	...
	if [ "$(dbus get phddns_enable)" == "1" -a -f "/jffs/softcenter/scripts/phddns_config.sh" ];then
		echo_date "重新开启插件..."
		/jffs/softcenter/scripts/phddns_config.sh restart
	fi
