...
install_now(){
	# default value
	local TITLE="灵缇游戏加速插件"
	local DESCR="灵缇游戏加速插件"
	local PLVER=$(cat ${DIR}/version)

	# stop before install
	if [ "$(dbus get lingti_enable)" == "1" -a -f "/jffs/softcenter/scripts/lingti_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/lingti_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/bin/lingti* >/dev/null 2>&1
	rm -rf /jffs/softcenter/res/icon-lingti.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/lingti_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_lingti.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_lingti.asp >/dev/null 2>&1
	...
	[ ! -L "/jffs/softcenter/init.d/S99lingti.sh" ] && ln -sf /jffs/softcenter/scripts/lingti_config.sh /jffs/softcenter/init.d/S99lingti.sh
	# Permissions
	...
	dbus set lingti_bin_version="1.4.2"
	# start after install
	if [ "$(dbus get lingti_enable)" == "1" -a -f "/jffs/softcenter/scripts/lingti_config.sh" ];then
		echo_date "重新开启插件..."
		/jffs/softcenter/scripts/lingti_config.sh start
	fi
