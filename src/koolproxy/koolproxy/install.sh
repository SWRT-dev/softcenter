...
install_now(){
	# default value
	local TITLE="koolproxy"
	local DESCR="koolproxy"
	local PLVER=$(cat ${DIR}/version)

	# stop before install
	if [ "$(dbus get koolproxy_enable)" == "1" -a -f "/jffs/softcenter/koolproxy/kp_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/koolproxy/kp_config.sh stop
	fi
	...
	if [ "$(dbus get koolproxy_enable)" == "1" -a -f "/jffs/softcenter/koolproxy/kp_config.sh" ];then
		echo_date "重新开启插件..."
		/jffs/softcenter/koolproxy/kp_config.sh restart
	fi
