...
install_now(){
	# default value
	local TITLE="多ddns合一"
	local DESCR="支持Alidns(阿里云) Dnspod(腾讯云) Cloudflare 华为云 Callback"
	local PLVER=$(cat ${DIR}/version)

	# stop before install
	if [ "$(dbus get ddns_enable)" == "1" -a -f "/jffs/softcenter/scripts/ddns_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/ddns_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/res/icon-ddns.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/ddns_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_ddns.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_ddns.asp >/dev/null 2>&1
	find /jffs/softcenter/init.d -name "*ddns*" | xargs rm -rf	
	...
	chmod +x /jffs/softcenter/scripts/uninstall_ddns.sh
	chmod +x /jffs/softcenter/bin/ddns*
	chmod +x /jffs/softcenter/init.d/*ddns*
	...
	if [ "$(dbus get ddns_enable)" == "1" -a -f "/jffs/softcenter/scripts/ddns_config.sh" ];then
		echo_date "重新开启插件..."
		/jffs/softcenter/scripts/ddns_config.sh restart
	fi
