...
fun_crontab(){
	...
	if [ "${frpc_cron_hour_min}" == "min" ]; then
		[ "${frpc_cron_type}" == "watch" ] && cru a frpc_monitor "*/${frpc_cron_time} * * * * pidof frpc >/dev/null || /jffs/softcenter/scripts/frpc_config.sh watch"
		[ "${frpc_cron_type}" == "start" ] && cru a frpc_monitor "*/${frpc_cron_time} * * * * /jffs/softcenter/scripts/frpc_config.sh start"
	elif [ "${frpc_cron_hour_min}" == "hour" ]; then
		[ "${frpc_cron_type}" == "watch" ] && cru a frpc_monitor "0 */${frpc_cron_time} * * * pidof frpc >/dev/null || /jffs/softcenter/scripts/frpc_config.sh watch"
		[ "${frpc_cron_type}" == "start" ] && cru a frpc_monitor "0 */${frpc_cron_time} * * * /jffs/softcenter/scripts/frpc_config.sh start"
	fi
}
# 创建日志文件目录及链接
logfile_prepare(){
	if [ "$frpc_customize_conf"x = "1"x ];then
		local a b c
		# For TOML
...
onstop(){
	...
	fun_nat_start
	[ -n "$(cru l | grep frpc_monitor)" ] && cru d frpc_monitor
	rm -f ${Conf_FILE} ${Visitor_Con}
}
#启动进程
onRun(){
	echo "【$(TZ=UTC-8 date -R +%Y年%m月%d日\ %X)】：启动 Frpc..." >>"${logFile_lnk}"
	# export GOGC=40
	
	# 启动时sh -c要执行的命令前加exec，使父进程/bin/sh被子进程frp替换，以免pid不一样出现异常
	start-stop-daemon -S -q -b -m -p ${PID_FILE} -a /bin/sh -- -c "exec ${BIN} -c ${Conf_FILE} >>${logFile_lnk} 2>&1"
}
# 开启frpc插件
fun_start_stop(){
	#是否禁用
	[ "${frpc_enable}"x = "1"x ] || { onstop; return 1; }
	...
	if [ "$frpc_customize_conf"x = "1"x ];then
		...
		for nu in ${server_nu}
		do
			eval array_name=\$frpc_subname_node_$nu
			eval array_type=\$frpc_proto_node_$nu
			eval array_localIP=\$frpc_localhost_node_$nu
			eval array_localPort=\$frpc_localport_node_$nu
			eval array_remotePort=\$frpc_remoteport_node_$nu
			eval array_customDomains=\$frpc_subdomain_node_$nu
			eval array_transport__useEncryption=\$frpc_encryption_node_$nu
			eval array_transport__useCompression=\$frpc_gzip_node_$nu
