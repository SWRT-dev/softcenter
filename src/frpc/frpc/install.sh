...
install_now(){
	...
	# stop before install
	if [ "$(dbus get frpc_enable)" == "1" -a -f "/jffs/softcenter/scripts/frpc_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/frpc_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/bin/frpc >/dev/null 2>&1
	rm -rf /jffs/softcenter/res/icon-frpc.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/frpc_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_frpc.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_frpc.asp >/dev/null 2>&1
