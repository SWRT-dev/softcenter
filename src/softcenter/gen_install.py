...
parent_path = os.path.realpath(os.path.join(curr_path, "..", "..", arch))
...
def work_parent():
    ignore_paths = frozenset(["maintain_files", "softcenter", "qiandao", "speedup", "syncthing"])
...
if stage == "stage1":
    ...
    with codecs.open(os.path.join(curr_path, "..", "..", arch, "softcenter", "config.json.js"), "r", "utf-8") as fc:
        conf = json.loads(fc.read())
        gmodules["version"] = conf["version"]
        gmodules["md5"] = conf["md5"]
        gmodules["tar_url"] = conf["tar_url"]
        gmodules["home_url"] = conf["home_url"]

        with codecs.open(os.path.join(curr_path, "..", "..", arch, "softcenter", "app.json.js"), "w", "utf-8") as fw:
