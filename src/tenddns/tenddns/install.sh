...
install_now(){
	...
	# stop before install
	if [ "$(dbus get tenddns_enable)" == "1" -a -f "/jffs/softcenter/scripts/tenddns_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/tenddns_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/res/icon-tenddns.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/tenddns_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_tenddns.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_tenddns.asp >/dev/null 2>&1
	find /jffs/softcenter/init.d -name "*tenddns*" | xargs rm -rf	
	...
	if [ "$(dbus get tenddns_enable)" == "1" -a -f "/jffs/softcenter/scripts/tenddns_config.sh" ];then
		echo_date "重新开启插件..."
		/jffs/softcenter/scripts/tenddns_config.sh start
	fi
