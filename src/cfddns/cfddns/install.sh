...
install_now(){
	...
	# stop before install
	if [ "$(dbus get cfddns_enable)" == "1" -a -f "/jffs/softcenter/scripts/cfddns_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/cfddns_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/res/icon-cfddns.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/cfddns_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_cfddns.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_cfddns.asp >/dev/null 2>&1
	...
	if [ "$(dbus get cfddns_enable)" == "1" -a -f "/jffs/softcenter/scripts/cfddns_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/cfddns_config.sh start
	fi
