...
install_now(){
	...
	# stop before install
	if [ "$(dbus get baidupcs_enable)" == "1" -a -f "/jffs/softcenter/scripts/baidupcs_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/baidupcs_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/res/icon-baidupcs.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/baidupcs_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_baidupcs.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_baidupcs.asp >/dev/null 2>&1
	find /jffs/softcenter/init.d -name "*baidupcs*" | xargs rm -rf	
	...
	if [ "$(dbus get baidupcs_enable)" == "1" -a -f "/jffs/softcenter/scripts/baidupcs_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/baidupcs_config.sh start
	fi
