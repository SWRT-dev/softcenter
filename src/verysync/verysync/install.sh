...
install_now(){
	...
	# stop before install
	if [ "$(dbus get verysync_enable)" == "1" -a -f "/jffs/softcenter/scripts/verysync_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/verysync_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/res/icon-verysync.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/verysync_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_verysync.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_verysync.asp >/dev/null 2>&1
	find /jffs/softcenter/init.d -name "*verysync*" | xargs rm -rf	

	# install file
	echo_date "安装插件相关文件..."
	cd /tmp
	cp -rf /tmp/${module}/init.d/* /jffs/softcenter/init.d/
