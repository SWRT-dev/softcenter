...
install_now(){
	...
	# stop before install
	if [ "$(dbus get acme_enable)" == "1" -a -f "/jffs/softcenter/scripts/acme_config.sh" ];then
		echo_date "安装前先关闭插件..."
		/jffs/softcenter/scripts/acme_config.sh stop
	fi

	# remove before install
	rm -rf /jffs/softcenter/res/icon-acme.png >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/acme_* >/dev/null 2>&1
	rm -rf /jffs/softcenter/scripts/uninstall_acme.sh >/dev/null 2>&1
	rm -rf /jffs/softcenter/webs/Module_acme.asp >/dev/null 2>&1
	find /jffs/softcenter/init.d -name "*acme*" | xargs rm -rf	
	...
	if [ "$(dbus get acme_enable)" == "1" -a -f "/jffs/softcenter/scripts/acme_config.sh" ];then
		echo_date "重新开启插件..."
		/jffs/softcenter/scripts/acme_config.sh start
	fi
